...
class CropHealthApp:
    ...
    def create_header(self):
        ...
        buttons = [
            ...
            ("⚙ Settings", self.show_settings, self.colors['warning']),
    ...
    def create_dashboard(self):
        ...
        stats = [
            ...
            ("🌡 Temperature", "26°C", self.colors['warning'], "Within range"),
            ("☀ Sunlight", "Optimal", self.colors['success'], "6-8 hours daily")
        ]
    ...
    def create_settings_tab(self):
        ...
        tk.Label(main_frame, text="⚙ System Settings",
                 font=('Arial', 24, 'bold'),
                 bg=self.colors['background'], fg=self.colors['primary']).pack(pady=20)
    ...
    def start_camera(self):
        try:
            ...
            self.cam_btn.config(text="⏹ Stop Camera", bg=self.colors['danger'])
...
📊 OVERALL HEALTH SCORE: {health_score}% {'✅' if health_score > 80 else '⚠'}

🔬 NUTRIENT ANALYSIS:
   • Nitrogen: {nutrients['Nitrogen']}% {'✅' if nutrients['Nitrogen'] > 75 else '⚠'}
   • Phosphorus: {nutrients['Phosphorus']}% {'✅' if nutrients['Phosphorus'] > 70 else '⚠'}
   • Potassium: {nutrients['Potassium']}% {'✅' if nutrients['Potassium'] > 75 else '⚠'}
   • Calcium: {nutrients['Calcium']}% {'✅' if nutrients['Calcium'] > 65 else '⚠'}

⚠ DISEASE DETECTION: {'None detected ✅' if random.random() > 0.3 else 'Early blight detected ⚠'}
